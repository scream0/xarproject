...
CREATE POLICY "Users can cancel their own pending orders."
  ...
  WITH CHECK (
    -- Only allow the status to be changed to 'cancelled'
    status = 'cancelled' AND
    -- and only from a pending or paid state
    (SELECT status FROM public.orders WHERE id = id) IN ('pending', 'paid')
  );
...
DROP POLICY "Admins can access all orders" ON public.orders;
