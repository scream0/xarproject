  ...
  try {
    ...
    if (!userId) {
      ...
      const { data, error, count } = await supabaseAdmin
        .from("orders")
        .select("id,user_id,status,amount,shipping_cost,discount_amount,tax_amount,payment_type,customer_name,customer_email,customer_phone,shipping_receipt_number,shipping_address,shipping_detail,created_at,updated_at, items:order_items(*)", { count: "exact" })
    ...
    const { data: ordersData, error: ordersError } = await supabaseAdmin
      .from("orders")
      .select("id,user_id,status,amount,shipping_cost,discount_amount,tax_amount,payment_type,customer_name,customer_email,customer_phone,shipping_receipt_number,shipping_address,shipping_detail,notes,created_at,updated_at, items:order_items(*)")
      .eq("user_id", userId)
      .order("created_at", { ascending: false });
