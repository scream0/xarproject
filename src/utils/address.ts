...
export function normalizeAddress(addr: any) {
...
export function formatAddressDisplay(addr: any) {
  if (!addr) return "Belum diatur";
  if (typeof addr === "string") return addr;

  const locationName = [addr.district ? `Kec. ${addr.district}` : "", addr.city, addr.province].filter(Boolean).join(", ");
  const addressParts = [
    addr.street,
    locationName,
    addr.postalCode ? `(${addr.postalCode})` : "",
    addr.notes ? `[Patokan: ${addr.notes}]` : "",
  ].filter(Boolean);
...
export function pickPrimaryAddress(addresses: any[] = []) {
  return (
