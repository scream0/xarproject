...
export default function AdminReturns() {
  ...
  const openPrompt = (id, action) => {
    setPromptModal({
      ...
      id,
      action,
      message: action === 'reject' ? "Masukkan alasan penolakan:" : "Masukkan catatan persetujuan (opsional):",
            ...
            <tbody>
              {returns.map((r) => (
                <tr key={r.id}>
                  ...
                  <td>{r.order_id}</td>
                  <td>
                    {r.profiles?.full_name || r.profiles?.username || 'User'} <br/>
                    <small>{r.profiles?.email}</small>
                  </td>
                  <td>
                    <b>{r.reason}</b>
