...
export default function OperationsCenter() {
  ...
  if (loading) return <p className={styles.state}>Loading operational workspace…</p>;

  return <div className={styles.center}>
    <section className={styles.hero}><div><p>Operations workspace</p><h2>See the next action before it becomes urgent.</h2></div><button onClick={exportMonthlyReport}>Download monthly report</button></section>
    <section className={styles.notificationGrid}>
      <Notice count={orders.filter((o) => o.status === "pending").length} label="Payment confirmations needed" />
      <Notice count={lowStock.length} label="Low-stock variants" />
      <Notice count={pendingReviews.length} label="New reviews to review" />
      <Notice count={soldOut.length} label="Out-of-stock variants" danger />
    </section>
    <section className={styles.grid}>
      <Panel title="Inventory & stock alerts" action={`${lowStock.length + soldOut.length} items need attention`}><div className={styles.list}>{[...soldOut, ...lowStock].slice(0, 6).map((item) => <div className={styles.row} key={`${item.name}-${item.variant}`}><span><b>{item.name}</b><small>{item.variant} · reorder suggestion: {Math.max(10, 15 - item.stock)} units</small></span><strong className={item.stock === 0 ? styles.danger : styles.warning}>{item.stock === 0 ? "Out of stock" : `${item.stock} left`}</strong></div>)}{!lowStock.length && !soldOut.length && <p className={styles.empty}>All tracked variants are above the alert threshold.</p>}</div></Panel>
    ...
    <section className={styles.automationSection}><Panel title="Team access" action={`${team.filter((member) => member.role !== "customer").length} operational users`}><div className={styles.list}>{team.filter((member) => member.role !== "customer").slice(0,5).map((member)=><div className={styles.row} key={member.id}><span><b>{member.name}</b><small>{member.email}</small></span><select value={member.role} onChange={(e)=>updateRole(member.id,e.target.value)} className={styles.roleSelect}><option value="admin">Admin</option><option value="staff">Staff</option><option value="customer">Customer</option></select></div>)}{!team.length&&<p className={styles.empty}>No team members found.</p>}</div></Panel></section>
    <section className={styles.automationSection}><Panel title="Payment reconciliation" action={`${reconciliation.summary.pendingCount || 0} payments need review`}><div className={styles.list}>{reconciliation.pending.slice(0,4).map((payment)=><div className={styles.row} key={payment.id}><span><b>{payment.id}</b><small>{payment.customer} · {money(payment.amount)} · {payment.paymentType}</small></span><strong className={styles.warning}>Pending</strong></div>)}{!reconciliation.pending.length&&<p className={styles.empty}>No pending payments to reconcile.</p>}</div></Panel></section>
    <section className={styles.automationSection}><Panel title="Supplier & purchase orders" action={`${procurement.orders.filter((order) => order.status !== "received").length} open POs`}><div className={styles.list}>{procurement.orders.slice(0, 4).map((order) => <div className={styles.row} key={order.id}><span><b>{order.item}</b><small>{procurement.suppliers.find((supplier) => supplier.id === order.supplierId)?.name || "Supplier"} · {order.quantity} units</small></span><strong className={order.status === "received" ? styles.ok : styles.warning}>{order.status}</strong></div>)}{!procurement.orders.length && <p className={styles.empty}>Create supplier and purchase orders via the procurement API.</p>}</div></Panel></section>
    <section className={styles.automationSection}>
      <Panel title="Automation rules" action="Operational safeguards">{rules.map((rule) => <div className={styles.rule} key={rule.id}><div><b>{rule.name}</b><small>{rule.description}</small></div><label><input type="checkbox" checked={rule.enabled} onChange={(e) => saveRules(rules.map((item) => item.id === rule.id ? { ...item, enabled: e.target.checked } : item))} /> Active</label><input aria-label={`${rule.name} setting`} value={rule.value} onChange={(e) => setRules(rules.map((item) => item.id === rule.id ? { ...item, value: e.target.value } : item))} onBlur={() => saveRules(rules)} /></div>)}{!rules.length && <p className={styles.empty}>Loading rules…</p>}</Panel>
    </section>    <section className={styles.promoSection}><Panel title="Promo & campaign management" action="Campaign draft"><div className={styles.formGrid}><label>Banner promo<input value={promo.banner} onChange={(e) => setPromo({ ...promo, banner: e.target.value })} /></label><label>Voucher code<input value={promo.voucher} onChange={(e) => setPromo({ ...promo, voucher: e.target.value.toUpperCase() })} /></label><label>Discount (%)<input type="number" value={promo.discount} onChange={(e) => setPromo({ ...promo, discount: e.target.value })} /></label><label>Schedule<input value={promo.schedule} onChange={(e) => setPromo({ ...promo, schedule: e.target.value })} /></label></div><button className={styles.saveButton} onClick={() => toast.success("Campaign draft saved for this session.")}>Save campaign draft</button></Panel>
      <Panel title="Storefront checklist" action="Edit in Settings"><ul className={styles.checklist}><li>Hero and promo banner content</li><li>Homepage product sections</li><li>Social proof and testimonials</li><li>Contact, shipping and policy information</li></ul></Panel>
    </section>
    <section className={styles.analytics}><Panel title="Lightweight analytics" action="Current data"><div className={styles.metrics}><Metric label="Traffic overview" value={`${orders.length} order sessions`} /><Metric label="Best seller" value={analytics.topProduct} /><Metric label="Sales by category" value={analytics.topCategory} /><Metric label="Peak sales hour" value={analytics.peakHour} /></div></Panel></section>
  </div>;
}
