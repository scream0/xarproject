...
export default function AdminChatView({ onUnreadCountChange }: any) {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const usersList = chatsData.map((chat: any) => ({
        id: chat.user_id,
      ...
      })).sort((a, b: any) => new Date(b.lastMessageAt) - new Date(a.lastMessageAt));

      setUsers(usersList);

      // Emit total unread count to parent
      if (onUnreadCountChange) {
        const totalUnread = usersList.reduce((sum, u: any) => sum + (u.unreadCount || 0), 0);
