...
/**
 * Input pencarian kota/kabupaten/area pengiriman (Biteship) yang reusable.
 *
 * Props:
 *  - value        (string) Teks kota yang sedang dipilih (mis. "Kab. Bandung")
 *  - cityId       (string) ID kota/area yang sedang dipilih
...
export function CitySearchInput({
  ...
  const search = useCallback(async (q) => {
    ...
    setLoading(true);
    try {
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || "") + `/api/ongkir/cities?query=${encodeURIComponent(trimmed)}`,
      );
